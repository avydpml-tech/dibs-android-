...
func add_history_row_event(eventData):
	...
	if eventData.event_id == "dialogic_002" and eventData.get("type") == 0 and not logArrivals:
		return
	
	if eventData.event_id == "dialogic_002" and eventData.get("type") == 1 and not logExits:
		return
