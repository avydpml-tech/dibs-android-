...
func _on_h_scroll_gui_input(event):
	if event is InputEventMouseButton:
		var v = (h_scroll.max_value - h_scroll.min_value) * 0.01
...
func _on_v_scroll_gui_input(event):
	if event is InputEventMouseButton:
		var v = (v_scroll.max_value - v_scroll.min_value) * 0.01
		match event.button_index:
			BUTTON_WHEEL_UP:
				v_scroll.value -= v
			BUTTON_WHEEL_DOWN:
...
func _draw():
	...
	content.rect_pivot_offset = get_scroll_rect().size / 2.0
					...
					if _is_connecting:
						...
						if _current_connection:
							...
							for i in current_layer.content_nodes.get_child_count():
								var child = current_layer.content_nodes.get_child(current_layer.content_nodes.get_child_count() - 1 - i)
