...
func create_import_preset_setting() -> void :
	if ProjectSettings.has_setting(_IMPORT_PRESET_KEY) and (ProjectSettings.get_setting(_IMPORT_PRESET_KEY) as Dictionary).size() > 0:
		return
...
func create_import_file(data: Dictionary) -> void :
	if not ProjectSettings.has_setting(_IMPORT_PRESET_KEY):
