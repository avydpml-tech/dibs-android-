...
func get_exec_flags(editor_path: String, script_path: String, line_number: int) -> PoolStringArray:
	...
	if editor_path.ends_with("code.cmd") or editor_path.ends_with("code.exe") or editor_path.ends_with("Code.exe"):
...
func populate_settings() -> void :
	...
	if not ignore_paths_field.is_connected("text_changed", self, "_on_ignore_paths_changed"):
